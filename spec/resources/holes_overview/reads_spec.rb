require 'rails_helper'

RSpec.describe HolesOverviewResource, type: :resource do
  describe 'serialization' do
    let!(:holes_overview) { create(:holes_overview) }

    it 'works' do
      render
      data = jsonapi_data[0]
      expect(data.id).to eq(holes_overview.id)
      expect(data.jsonapi_type).to eq('holes_overviews')
    end
  end

  describe 'filtering' do
    let!(:holes_overview1) { create(:holes_overview) }
    let!(:holes_overview2) { create(:holes_overview) }

    context 'by id' do
      before do
        params[:filter] = { id: { eq: holes_overview2.id } }
      end

      it 'works' do
        render
        expect(d.map(&:id)).to eq([holes_overview2.id])
      end
    end
  end

  describe 'sorting' do
    describe 'by id' do
      let!(:holes_overview1) { create(:holes_overview) }
      let!(:holes_overview2) { create(:holes_overview) }

      context 'when ascending' do
        before do
          params[:sort] = 'id'
        end

        it 'works' do
          render
          expect(d.map(&:id)).to eq([
            holes_overview1.id,
            holes_overview2.id
          ])
        end
      end

      context 'when descending' do
        before do
          params[:sort] = '-id'
        end

        it 'works' do
          render
          expect(d.map(&:id)).to eq([
            holes_overview2.id,
            holes_overview1.id
          ])
        end
      end
    end
  end

  describe 'sideloading' do
    # ... your tests ...
  end
end
